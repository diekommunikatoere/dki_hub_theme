...
type FAQAction = { type: "SET_LOADING"; payload: boolean } | { type: "SET_ERROR"; payload: string | null } | { type: "SET_CONFIG"; payload: WordPressConfig } | { type: "SET_SECTIONS"; payload: FAQSection[] } | { type: "ADD_SECTION"; payload: FAQSection } | { type: "UPDATE_SECTION"; payload: FAQSection } | { type: "DELETE_SECTION"; payload: number } | { type: "REORDER_SECTIONS"; payload: FAQSection[] } | { type: "SET_FAQ_ITEMS"; payload: { sectionId: number; items: FAQItem[] } } | { type: "ADD_FAQ_ITEM"; payload: FAQItem } | { type: "UPDATE_FAQ_ITEM"; payload: FAQItem } | { type: "DELETE_FAQ_ITEM"; payload: { itemId: number; sectionId: number } } | { type: "REORDER_FAQ_ITEMS"; payload: { sectionId: number; items: FAQItem[] } };
	...
	switch (action.type) {
		...
		case "UPDATE_FAQ_ITEM":
			return {
				...
				faqItems: {
					...state.faqItems,
					[action.payload.sectionId]: (state.faqItems[action.payload.sectionId] || []).map((item) => (item.id === action.payload.id ? action.payload : item)),
				},
			};
		...
		case "DELETE_FAQ_ITEM":
			return {
				...
				faqItems: {
					...state.faqItems,
					[action.payload.sectionId]: (state.faqItems[action.payload.sectionId] || []).filter((item) => item.id !== action.payload.itemId),
				},
			};
...
export const FAQProvider: React.FC<{ children: ReactNode; config: WordPressConfig }> = ({ children, config }) => {
